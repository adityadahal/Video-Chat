    ...
    this.vidyoConnector.RegisterMessageEventListener({
      ...
      onChatMessageAcknowledged: function (message: any) {
        ...
        if (
          ...
          let msg =
            thisClass.chatMessages[thisClass.chatMessages.length - 1].message;
          thisClass.chatMessages[thisClass.chatMessages.length - 1].message =
            msg + '\n' + message.body;
        } else {
          // Create a new chat bubble and append the chat bubble to the message-box
          thisClass.chatMessages.push({
            name: 'You',
            time: timestamp,
        ...
        thisClass.chatData.lastChatParticipant = 'You';
        thisClass.chatData.lastChatParticipantId =
          thisClass.chatData.localParticipantId;
        thisClass.chatData.lastChatTime = timestamp;
      ...
      onChatMessageReceived: function (participant: any, message: any) {
        ...
        if (
          participant.id == thisClass.chatData.lastChatParticipantId &&
          timestamp == thisClass.chatData.lastChatTime
        ) {
          let msg =
            thisClass.chatMessages[thisClass.chatMessages.length - 1].message;
          thisClass.chatMessages[thisClass.chatMessages.length - 1].message =
        ...
        if (
          !thisClass.rightPaneExpanded ||
          (thisClass.rightPaneExpanded && !thisClass.chatData.chatOpen)
        ) {
